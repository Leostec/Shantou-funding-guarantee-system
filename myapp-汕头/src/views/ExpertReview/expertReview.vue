    ...
    >
      <a-form layout="vertical">
        <a-form-item label="专家意见">
          <a-textarea v-model:value="editForm.expert_opinion" rows="4" />
        </a-form-item>
        <a-form-item label="专家测算额度">
          <a-input-number v-model:value="editForm.expert_amount" style="width: 100%;" />
        </a-form-item>
      </a-form>
    </a-modal>
  </div>
...
const openEdit = (record: RecordItem) => {
  ...
  editForm.value = {
    ...
    expert_amount: record.expert_amount || null,
  };
...
const submitEdit = async () => {
  ...
  saving.value = true;
  try {
    await axios.put(`http://localhost:8989/loan-application/${currentRecordId.value}`, {
