<template>
  <div class="page">
    ...
    <!-- 预测结果弹窗 -->
    <a-modal
      v-model:visible="predictionModalVisible"
      title="预测完成"
      width="900"
      centered
      ok-text="确定"
      cancel-text=""
      :footer="null"
      @cancel="predictionModalVisible = false"
    >
      <div style="max-height: 60vh; overflow: auto; padding: 10px; white-space: pre-wrap; word-wrap: break-word;">
        <div style="margin-bottom: 10px; font-weight: bold;">
          预测额度：{{ predictionResult.amount ?? '未识别' }}万元
        </div>
        <div style="border-top: 1px solid #e8e8e8; padding-top: 10px;">
          {{ predictionResult.report || '无' }}
        </div>
      </div>
      <template #footer>
        <a-button type="primary" @click="predictionModalVisible = false">确定</a-button>
      </template>
    </a-modal>
...
const handlePredict = async (record: RecordItem) => {
  const id = record.id || record.project_number || record.project_enterpriseid;
  if (!id) {
    message.error('????ID?????');
    return;
  }
  ...
  try {
    const resp = await axios.post('http://127.0.0.1:5001/demo', record);
    let predictionText = '';
    if (resp.data) {
    ...
    // 获取完整的评估报告
    const fullReport = resp.data?.['大模型判断'] || resp.data?.judgment || predictionText;
